...
import { VKV_WORDS, VKV_PHRASES, VKV_SENTENCES } from '../constants';
import { Type, MessageSquare, AlignLeft } from 'lucide-react';

const VkVManager: React.FC = () => {
        ...
        <div className="bg-white p-1 rounded-2xl shadow-sm border border-gray-100 flex gap-1">
            <button
                ...
                className={`flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-bold transition-all ${
                    activeTab === 'words' ? 'bg-orange-500 text-white shadow-md' : 'text-gray-500 hover:bg-gray-50'
                }`}
            >
      ...
      <div className="animate-fade-in">
        {activeTab === 'words' && (
            <div className="text-center">
                <h2 className="text-2xl font-bold text-gray-700 mb-6">Mari Belajar Perkataan V+KV</h2>
                <InteractiveList mode="words" data={VKV_WORDS} />
            </div>
        )}
        {activeTab === 'phrases' && (
             <div className="text-center">
                <h2 className="text-2xl font-bold text-gray-700 mb-6">Mari Baca Frasa</h2>
                <InteractiveList mode="phrases" data={VKV_PHRASES} />
             </div>
        )}
        {activeTab === 'sentences' && (
            <div>
                 <SentenceCards data={VKV_SENTENCES} />
            </div>
        )}
      </div>
    </div>
