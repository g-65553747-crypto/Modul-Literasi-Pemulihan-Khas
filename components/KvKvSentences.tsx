...
import { ChevronLeft, ChevronRight, Volume2, PlayCircle } from 'lucide-react';

const KvKvSentences: React.FC = () => {
  const [currentIndex, setCurrentIndex] = useState(0);

  const handleNext = () => {
    setCurrentIndex((prev) => (prev + 1) % KVKV_SENTENCES.length);
  };

  const handlePrev = () => {
    setCurrentIndex((prev) => (prev - 1 + KVKV_SENTENCES.length) % KVKV_SENTENCES.length);
  };

  const currentItem = KVKV_SENTENCES[currentIndex];
  ...
  return (
    <div className="flex flex-col items-center justify-center py-10 px-4 w-full max-w-4xl mx-auto">
      <div className="text-center mb-6">
          <h2 className="text-2xl font-bold text-gray-700">Mari Baca Ayat</h2>
          <p className="text-gray-400">Kad Imbasan {currentIndex + 1} / {KVKV_SENTENCES.length}</p>
      </div>
